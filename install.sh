...
function Build-Containers {
    ...
    read -n 1 -p 'Do you want to build containers? (y/n) ' Continue
    if [[ "$Continue" == "y" ]]
    then
        echo "Downloading images and dependencies."
...
{
     read -n 1 -p 'Do you want to start Pulsar now? (y/n) ' Continue
     if [[ "$Continue" == "y" ]]
     then
...
read -n 1 -p 'Do you want to countinue? (y/n) ' Continue
if [[ "$Continue"=="y" ]]
then
    Check-DockerCompose
    Gen-SSHKey
